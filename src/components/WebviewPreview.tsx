...
const WebviewPreviewComponent: React.FC<WebviewPreviewProps> = ({
  ...
  return (
    <div 
      ...
      tabIndex={-1}
      role="region"
      aria-label={t('preview.webPreview')}
    >
      {/* Browser Top Bar */}
            ...
            {onToggleMaximize && (
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    {isMaximized ? t('preview.exitFullscreen') : t('preview.enterFullscreen')}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
      ...
      <div className="flex-1 relative bg-background" ref={contentRef}>
        ...
        {hasError ? (
          <div className="flex flex-col items-center justify-center h-full p-8">
            ...
            <h3 className="text-lg font-semibold mb-2">{t('preview.loadFailed')}</h3>
            <p className="text-sm text-muted-foreground text-center mb-4">
              {errorMessage || t('preview.loadFailedDescription')}
            </p>
          ...
          <div className="flex flex-col items-center justify-center h-full p-8 text-foreground">
            ...
            <h3 className="text-xl font-semibold mb-3">{t('preview.enterUrlToPreview')}</h3>
            <p className="text-sm text-muted-foreground text-center mb-6 max-w-md">
              {t('preview.enterUrlHint')}
            </p>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{t('preview.tryEntering')}</span>
              <code className="px-2 py-1 bg-muted/50 text-foreground rounded font-mono text-xs">localhost:3000</code>
              <span>{t('preview.orAnyUrl')}</span>
            </div>
          </div>
        )}
      </div>
