      ...
      >
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-lg font-semibold">{t('dashboard.usage')}</h1>
              <p className="text-xs text-muted-foreground">
                {t('dashboard.trackUsage')}
              </p>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center max-w-md">
              ...
              <Button onClick={loadUsageStats} size="sm">{t('dashboard.tryAgain')}</Button>
            </div>
          </div>
        ) : stats ? (
          <motion.div
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('dashboard.totalCost')}</p>
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('dashboard.totalSessions')}</p>
                    <p className="text-2xl font-bold mt-1">
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('dashboard.avgCostPerSession')}</p>
                    <p className="text-2xl font-bold mt-1">
                      {formatCurrency(
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-5">
                ...
                <TabsTrigger value="models">{t('dashboard.byModel')}</TabsTrigger>
                <TabsTrigger value="projects">{t('dashboard.byProject')}</TabsTrigger>
                <TabsTrigger value="sessions">{t('dashboard.bySession')}</TabsTrigger>
              ...
              <TabsContent value="overview" className="space-y-4">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('dashboard.tokenBreakdown')}</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      <p className="text-xs text-muted-foreground">{t('dashboard.inputTokens')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_input_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('dashboard.outputTokens')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_output_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('dashboard.cacheWrite')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_cache_creation_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('dashboard.cacheRead')}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card className="p-6">
                    <h3 className="text-sm font-semibold mb-4">{t('dashboard.mostUsedModels')}</h3>
                        ...
                        <div key={model.model} className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {model.session_count} {t('dashboard.sessions')}
                            </span>
                          </div>
                  ...
                  <Card className="p-6">
                    <h3 className="text-sm font-semibold mb-4">{t('dashboard.topProjects')}</h3>
                    <div className="space-y-3">
              ...
              <TabsContent value="models">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('dashboard.usageByModel')}</h3>
                    ...
                    {stats.by_model.map((model) => (
                      <div key={model.model} className="space-y-2">
                        ...
                        <div className="grid grid-cols-4 gap-2 text-xs">
                          <div>
                            <span className="text-muted-foreground">{t('dashboard.input')}: </span>
                            <span className="font-medium">{formatTokens(model.input_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('dashboard.output')}: </span>
                            <span className="font-medium">{formatTokens(model.output_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('dashboard.cacheW')}: </span>
                            <span className="font-medium">{formatTokens(model.cache_creation_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('dashboard.cacheR')}: </span>
              ...
              <TabsContent value="projects">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('dashboard.usageByProject')}</h3>
                  <div className="space-y-3">
                      ...
                      <div key={project.project_path} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                        <div className="flex flex-col truncate">
                          ...
                          <div className="flex items-center space-x-3 mt-1">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {formatTokens(project.total_tokens)} {t('dashboard.tokens')}
                            </span>
                          </div>
                        </div>
              ...
              <TabsContent value="sessions">
                  <Card className="p-6">
                      <h3 className="text-sm font-semibold mb-4">{t('dashboard.usageBySession')}</h3>
                      <div className="space-y-3">
              ...
              <TabsContent value="timeline">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-6 flex items-center space-x-2">
                    ...
                    <span>{t('dashboard.dailyUsage')}</span>
                  </h3>
                                ...
                                <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                                  <div className="bg-background border border-border rounded-lg shadow-lg p-3 whitespace-nowrap">
                                    ...
                                    <p className="text-sm text-muted-foreground mt-1">
                                      {t('dashboard.cost')}: {formatCurrency(day.total_cost)}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      {formatTokens(day.total_tokens)} {t('dashboard.tokens')}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      {day.models_used.length} {t('dashboard.model')}{day.models_used.length !== 1 ? 's' : ''}
                                    </p>
                                  </div>
