...
export const MCPAddServer: React.FC<MCPAddServerProps> = ({
  ...
  const handleAddStdioServer = async () => {
    if (!stdioName.trim()) {
      onError(t("mcp.add.nameRequired"));
      return;
    }

    if (!stdioCommand.trim()) {
      onError(t("mcp.add.commandRequired"));
      return;
    }
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h3 className="text-base font-semibold">{t("mcp.add.title")}</h3>
      ...
      <Tabs value={transport} onValueChange={(v) => setTransport(v as "stdio" | "sse")}>
        ...
        <TabsContent value="stdio" className="space-y-6">
          <Card className="p-6 space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="stdio-name">{t("mcp.add.serverName")}</Label>
                <Input
                  id="stdio-name"
                  placeholder={t("mcp.add.serverNamePlaceholder")}
                  value={stdioName}
                  onChange={(e) => setStdioName(e.target.value)}
                />
                <p className="text-xs text-muted-foreground">
                  {t("mcp.add.serverNameDesc")}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="stdio-command">{t("mcp.add.command")}</Label>
                <Input
                  id="stdio-command"
                ...
                />
                <p className="text-xs text-muted-foreground">
                  {t("mcp.add.commandDesc")}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="stdio-args">{t("mcp.add.arguments")}</Label>
                <Input
                  id="stdio-args"
                  placeholder={t("mcp.add.argumentsPlaceholder")}
                  value={stdioArgs}
                  onChange={(e) => setStdioArgs(e.target.value)}
                  className="font-mono"
                />
                <p className="text-xs text-muted-foreground">
                  {t("mcp.add.argumentsDesc")}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="stdio-scope">{t("mcp.add.scope")}</Label>
                <SelectComponent
                  value={stdioScope}
        ...
        <TabsContent value="sse" className="space-y-6">
          <Card className="p-6 space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="sse-name">{t("mcp.add.serverName")}</Label>
                <Input
                  id="sse-name"
                  placeholder={t("mcp.add.serverNamePlaceholder")}
                  value={sseName}
              ...
              <div className="space-y-2">
                <Label htmlFor="sse-url">{t("mcp.add.url")}</Label>
                <Input
                  id="sse-url"
              ...
              <div className="space-y-2">
                <Label htmlFor="sse-scope">{t("mcp.add.scope")}</Label>
                <SelectComponent
                  value={sseScope}
      ...
      <Card className="p-4 bg-muted/30">
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            <span>{t("mcp.add.exampleCommands")}</span>
          </div>
