                    ...
                    if (content.type === "tool_use") {
                      ...
                      const renderToolWidget = () => {
                        ...
                        if (content.name?.startsWith("mcp__")) {
                          ...
                          // For sequential thinking, the thought comes from input, metadata from result
                          if (
                            content.name ===
                            ...
                            if (toolResult) {
                              try {
                                ...
                                if (typeof contentToParse === "string") {
                                  ...
                                  contentToParse &&
                                  typeof contentToParse === "object"
                                ) {
                                  if (contentToParse.text) {
                                    try {
                                      contentToParse = JSON.parse(
                                        contentToParse.text,
                                      );
                                    } catch (jsonError) {
                                      contentToParse = null;
                                    ...
                                    const firstText = contentToParse.find(
                                      (c: any) => c.text,
                                    );
                                    if (firstText?.text) {
                                      try {
                            ...
                            return (
                              <SequentialThinkingWidget
                                ...
                                thoughtNumber={
                                  input.thoughtNumber ||
                                  resultData?.thoughtNumber
                                }
                                totalThoughts={
                                  input.totalThoughts ||
                                  resultData?.totalThoughts
                                }
                                nextThoughtNeeded={
                                  input.nextThoughtNeeded ||
                                  resultData?.nextThoughtNeeded
                                }
                              />
                            );
                          }
                    ...
                    if (content.type === "tool_result") {
                      ...
                      if (reminderMatch) {
                        ...
                        const afterReminder = contentText
                          .substring(
                            (reminderMatch.index || 0) +
                              reminderMatch[0].length,
                          )
                          .trim();
