...
export function SessionOutputViewer({
  session,
  onClose,
  className,
}: SessionOutputViewerProps) {
        ...
        try {
          ...
          if (session.status === "running") {
            ...
            try {
              ...
              console.warn(
                "Failed to start streaming, will poll instead:",
                streamError,
              );
            }
          }
          ...
          console.warn(
            "Failed to load from JSONL, falling back to regular output:",
            err,
          );
        }
      }
  ...
  return (
    <>
      ...
      >
        <Card
          className={`h-full ${isFullscreen ? "rounded-none border-0" : ""}`}
        >
          ...
          <CardContent
            className={`${isFullscreen ? "h-[calc(100vh-120px)]" : "h-96"} p-0`}
          >
            {loading ? (
